/**
 * Reusable, high-performance Markdown renderer component for blog posts and technical deep dives.
 * Supports:
 * - ReactMarkdown + remarkGfm
 * - Prism syntax highlighting with language badges and copy button
 * - Mermaid diagram rendering
 * - Image lightbox modal for diagram zooming
 * - Accessible links and custom blockquotes
 */
...
export default function MarkdownRenderer({ content }: MarkdownRendererProps) {
  ...
  // Pre-process content to isolate Mermaid diagrams if needed
  const renderBlocks = () => {
    ...
    <div className="prose prose-invert max-w-none prose-pre:p-0 prose-pre:bg-transparent">
      {blocks.map((block, blockIdx) => {
        if (block.type === 'mermaid') {
          return (
            <div key={`block-${blockIdx}`} className="my-8 overflow-x-auto">
